...
//void SoftRender::DrawTriangle3D(SimpleVertex* vs_out, const WORD indices[3])
{
	...
	XMFLOAT3 p_bary_start, p_bary_start_x1,p_bary_start_y1;
...
#pragma omp parallel for schedule(dynamic)
 ...
 //		#pragma omp parallel for
		...
		for (int j = 0; j < scanHeight; ++j)
		{
			...
			bool cur_flag = IsInTriangle(p_bary);
			if (flag && !cur_flag)
			{
				break;
			}
...
inline XMFLOAT3 SoftRender::getBilinearFilteredPixelColor(Buffer<XMFLOAT3>& tex, double u, double v)
{
	...
	else
	{
		...
		float tex_x1_y1_G = tex(x+1, y + 1).x;
		float tex_x1_y1_B = tex(x+1, y + 1).x;
...
inline XMFLOAT4 SoftRender::transProSpace(const XMFLOAT3& p)
{
	XMVECTOR v;
...
inline XMFLOAT3 SoftRender::transWorldSpace(const XMFLOAT3& p)
{
	XMVECTOR v;
